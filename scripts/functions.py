		...
		for i in range(start,end+1):	
			if (i>=0 and i<limit and i<len(mapData.data)):
				for j in range(0,len(centroids)):
					...
					dist_to_bot = norm(point_of_index(mapData,i)-assigned_pt)
					if(mapData.data[i]==-1 and norm(point_of_index(mapData,i)-current_pt)<=r and dist_to_bot<=r):
						infoGain[j]-=1/(dist_to_bot+0.1) #this should be modified, subtract the area of a cell, not 1
	return infoGain
#________________________________________________________________________________
